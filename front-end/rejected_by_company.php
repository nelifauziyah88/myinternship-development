      ...
      <div class="form-container">
        <form id="internshipForm">
          ...
          <div id="uploadSection" style="display:none;">
            <div class="form-group">
              <label>Upload company reply letter</label>
...
// ======= SHOW FILE INPUT WHEN SELECT YES =======
document.getElementById("responseReceived").addEventListener("change", function () {
...
document.getElementById("internshipForm").addEventListener("submit", async function (e) {
  ...
  const response = document.getElementById("responseReceived").value;
  const fileInputEl = document.getElementById('fileInput');
const file = fileInputEl && fileInputEl.files.length > 0 ? fileInputEl.files[0] : null;
...
// ======= LOAD STATUS IF ALREADY REJECTED =======
async function loadLetterInfo() {
  try {
    const res = await fetch(`${apiBase}/student/letter/${id}`);
    const json = await res.json();
    ...
    if (data.acceptance_status === "REJECTED") {
      ...
      let html = `
        <div class="alert alert-danger">
          <strong>Internship rejected by company.</strong>
        </div>
      `;

      if (data.company_reply_letter) {
        html += `
          <a href="../${data.company_reply_letter}" target="_blank" class="btn btn-info">
            View Company Reply Letter
          </a>
        `;
      } else {
        html += `<p><i>No reply letter uploaded.</i></p>`;
      }

      html += `
        <br><br>
        <button onclick="window.location.href='approval_status.php'" class="btn btn-primary">
          Back to Status
        </button>
      `;
...
document.addEventListener("DOMContentLoaded", loadLetterInfo);
</script>
</body>
</html>